...
class Repository {
    ...
    /**
     * install_service_connection
     * 
     * accommodate `com_link_manager` module.
     * 
     * @param {object} instance - require wrapper instance to be interned into the bridge
     * @param {object} conf - necessary configuration -- the repository client will most likely be activated
     */
     ...
     * @param {object} instance -- may be the instance of the wrapper, but this module does bookkeping on that
     * @param {object} conf -- must have the activation information for the update
     */
    ...
    /**
     * init_newly_activated_repos
     * 
     * @param {Array} new_activations -- a list of keys for pulling configurations out of the configuratios map.
     */
     ...
     * 
     * The assumption with `store_local` is that a client will already have the meta descriptor of the file.
     * Hence, it has access to the pin ID.
     * 
     ...
     * @returns promise  -- resolves to true if the pin succeeds. false if an error occurs.
     */
     ...
     * 
     * @param {string} kind 
     * @param {object} conf -- configuration for the link constructor
     * @param {object} link_m -- instance of a link manager..
     * 
     * @returns boolean
     */
    set_link_manager(kind,conf,link_m) {
        try {
            let w_repo = this.repos[kind]
            if ( typeof w_repo.set_link_manager === 'function' ) {
                w_repo.set_link_manager(conf,link_m)
            }
            return true
        } catch (e) {
            console.log(e)
    ...
    set_link_mamagers(conf,link_m) {
        for ( let k in this.configs ) {
            if ( k in conf ) {
                this.set_link_manager(k,conf,link_m)
            }
        }
    }
